...
```text
mb-20260505-150405-a3f19c
tmm-20260505-150405-b812ef
```
...
zt install --with-ghostty-terminfo # copy xterm-ghostty terminfo to remote
...
remote `tmux`, optional local `zellij`, and optional Ghostty terminfo.

Ghostty:

`zt` launches `ssh` directly, so Ghostty's interactive shell `ssh` wrapper does
not run. When `zt` detects Ghostty, it forwards `COLORTERM`, `TERM_PROGRAM`, and
`TERM_PROGRAM_VERSION` through SSH. For the best remote rendering, install
Ghostty's terminfo on the remote host:

```bash
zt status
zt install --with-ghostty-terminfo
```

This copies the local `xterm-ghostty` terminfo entry to the remote host using
`infocmp -x xterm-ghostty | ssh HOST -- 'mkdir -p "$HOME/.terminfo" && TERMINFO="$HOME/.terminfo" tic -x -'`.
If the remote host already has the entry, the install step is skipped. The
remote host must have `tic` available.

For a more native workflow, use `zt ghostty SESSION`. It opens the remote
session in a real Ghostty window and returns immediately:

```bash
zt ghostty main
zt ghostty work
zt ghostty --print-command work
```
